...
- **Playbooks 1-6, 8-12, 15-19, 22, 25-26, 28-30, 32 y 34-42**: el "servidor" gestionado es la propia máquina (`localhost` con `ansible_connection=local`). Sin SSH, sin servidores remotos, sin permisos de administrador — todo ocurre dentro del directorio del proyecto.
...
| `playbooks/39_serial_a_fondo.yml` | **Despliegues por tandas a fondo: `serial`, `max_fail_percentage` y `any_errors_fatal`** — lo que decide cuántos servidores se rompen antes de que ansible pare. Sobre una flota FALSA de ocho alias de localhost en memoria (`add_host` + `connection: local`, sin Docker), medido contra ansible-core 2.21 y con varias sorpresas fijadas con `assert`: **`serial: 2`** hace 4 tandas de 2 y **`serial: [1, 3]`** una de 1 (el canario), luego de 3 y repite el último tamaño hasta acabar (`[1,3,3,3,3,3,3,1]` por host); **`max_fail_percentage: 50`** con un fallo entre dos NO salta (50 no supera 50) y los otros siete completan; **`max_fail_percentage: 30`** salta — y **medido**: la comprobación es **tras cada tarea**, no al final de la tanda (el compañero del caído ya no recibe la tarea siguiente), las tandas restantes no empiezan **y ansible termina el playbook entero** (rc 2, `NO MORE HOSTS LEFT`: la play posterior no corre), por lo que ese experimento va en una orden anidada; **`any_errors_fatal`** corta la TANDA del fallo (los compañeros se quedan sin las tareas restantes) pero **la tanda siguiente arranca de cero y corre entera** — es un freno por tanda, no por playbook (para parar todo: `max_fail_percentage: 0`); y **el host que falla sale del resto del playbook** (la primera versión hacía fallar al mismo nodo en dos plays y la segunda "no fallaba": ya no estaba; `meta: clear_host_errors` lo readmite), con las tandas calculadas sobre la lista ORIGINAL de hosts (serial 4 con nodo3 caído dio `[1,2,4]` y `[5,6,7,8]`). El rc final es 2 a propósito; el CI lo exige y comprueba la marca de veredicto. |
| `playbooks/40_inventario_en_caliente_a_fondo.yml` | **Inventario en caliente a fondo: `add_host`, `group_by`, `refresh_inventory` y `clear_host_errors`** — lo que cambia la lista de hosts *mientras* el playbook corre. Cuatro escenas con `assert`, medidas contra ansible-core 2.21 sobre una flota falsa de alias de `localhost`: **`add_host` corre UNA sola vez por play** aunque la play tenga tres hosts (es una acción "bypass host loop": con `name: extra-{{ inventory_hostname }}` aparece un único `extra-nodo1`) y el host y su grupo **se ven al instante** en `groups` dentro de la misma play (como destino de `hosts:` solo en las siguientes); **`group_by` corre por host** y agrupa según la variable de cada uno (`tier40_web = [nodo1, nodo2]`, `tier40_db = [nodo3]`); **`refresh_inventory`** — un fichero de inventario que cambia durante el playbook no se relee solo: `groups['dyn']` sigue en `dinA` hasta el `meta: refresh_inventory`, después `dinA,dinB` y la play siguiente corre en los dos… y **los hosts de `add_host` sobreviven al refresh** (2.21 los conserva, contra lo que suele creerse); y **`clear_host_errors`**, medido con dos órdenes anidadas idénticas salvo el meta: sin él el host caído desaparece de las plays siguientes y el playbook acaba con rc 2; con él **no vuelve en la misma play** (las tareas posteriores al meta siguen sin nodo2) pero **sí en la siguiente**, y el playbook acaba con **rc 0** aunque el recap siga diciendo `failed=1`. |
| `playbooks/41_delegacion_a_fondo.yml` | **Delegación a fondo: `delegate_to`, `run_once`, `delegate_facts` y el fan-out** — *dónde* corre de verdad cada tarea y *qué* variables ve. Cuatro escenas con `assert`, medidas contra ansible-core 2.21 sobre la flota falsa: **`run_once`** corre una sola vez, en el primer host de la play, y su `register` se copia a todos (los tres leen el mismo `ro.stdout`); **`delegate_to`** ejecuta en el delegado pero la plantilla se rinde con las variables del host *original* (`inventory_hostname` y `tier` siguen siendo los de quien pide; lo del delegado se lee por `hostvars[...]`, y `ansible_delegated_vars` existe solo dentro de la tarea delegada) — variables del que pide, conexión del que ejecuta; **`delegate_facts`**: un `set_fact` delegado se guarda en el host *original* salvo `delegate_facts: true`, que lo guarda en el delegado (y con tres escritores sobre nodo3 gana el último); **`delegate_to: localhost`** (el viejo `local_action`) corre una vez *por host*, no una vez (tres marcas), y el **fan-out** `loop: groups[...]` + `delegate_to: "{{ item }}"` + `run_once` corre una vez desde el primer host mandando cada vuelta a su destino, con las tres salidas visibles en todos. |
| `playbooks/42_estrategias_a_fondo.yml` | **Estrategias a fondo: `strategy: linear` frente a `free`, `throttle` y `forks`** — quién espera a quién, y cuánto. Cuatro escenas con `assert` sobre marcas de tiempo, medidas contra ansible-core 2.21 con un `nodo1` que duerme 3 s en la primera tarea: **linear** (la estrategia por defecto) avanza tarea a tarea y cada tarea espera a *todos* — nodo2 y nodo3 no llegaron a la tarea 2 hasta pasados los 3 s de nodo1, sin dormir ninguno; **free** deja que cada host avance por sus tareas — nodo2 y nodo3 acabaron las tres ~3 s antes que nodo1 (el precio: el orden se mezcla y "todos han llegado aquí" deja de existir); **`throttle: 1`** serializa una tarea sea cual sea la estrategia — 1 s por host = ≥ 3 s en total frente a ~1 s sin él; y **`forks: 1`** (orden anidada con `ANSIBLE_FORKS=1`) limita la *concurrencia*, no el orden — tres hosts que duermen 1 s tardan ≥ 3 s aunque la estrategia sea free, y las tareas se siguen entremezclando (la primera versión de la escena esperaba un round-robin ordenado; la medición dijo que no). Umbrales holgados (2.5 s frente a 3 s) para que un runner lento no invente fallos. |
